    ...
    stages {
        stage('Git checkout') {
            steps {
                ...
                sh 'git --version'
                git branch: 'main', url: 'https://github.com/joaferna/ci-cd-test.git'
            }
        }
        stage('Pull Docker images') {
            steps {
                ...
                sh "docker tag rocket.chat:5.0.3 ${params.ARTIFACTORY_URL}/rocket.chat:5.0.3"
            }
        }
            ...
            steps {
                script {
                    ...
                    if (params.MONGODB == 'REDEPLOY_MONGODB') {
                        sh "ssh -o StrictHostKeyChecking=no -i ${env.SERVER_LOGIN} ${params.SSH_USER}@${params.SERVER_FQDN} 'kubectl apply -f https://raw.githubusercontent.com/joaferna/ci-cd-test/k8s/mongo.yaml'"
                        sleep 90
                    ...
                    sh "ssh -o StrictHostKeyChecking=no -i ${env.SERVER_LOGIN} ${params.SSH_USER}@${params.SERVER_FQDN} 'kubectl apply -f https://raw.githubusercontent.com/joaferna/ci-cd-test/k8s/rocketchat.yaml'"
                    sleep 90
